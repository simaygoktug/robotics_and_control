...
#Gazebo – Robot Simülasyon Ortamı
#Fizik tabanlı simülasyon (dinamik, çarpışma, ağırlık)
#URDF/SDF ile robot tanımlama
#Sensor verisi üretme (kamera, lidar, imu)

sudo apt install gazebo11

#two_link_robot.urdf.xacro dosyasını oluştur

ros2 launch gazebo_ros empty_world.launch.py
ros2 run robot_state_publisher robot_state_publisher.py
...
class JointPublisher(Node):
    def __init__(self):
        ...
        self.pub1 = self.create_publisher(Float64, '/joint1_position_controller/command', 10)
        self.pub2 = self.create_publisher(Float64, '/joint2_position_controller/command', 10)
        self.timer = self.create_timer(0.1, self.send)

    def send(self):
        self.pub1.publish(Float64(data=1.0))
        self.pub2.publish(Float64(data=0.5))
...
#ROS2 – Gerçek Zamanlı Kontrol + Sensör Verisi İşleme
#Robot işletim sistemi: gerçek zamanlı mesajlaşma
#topic, service, action gibi iletişim protokolleri
#Kamera, IMU, lidar gibi verilerin kontrol kodlarıyla entegresi

sudo apt install ros-humble-desktop
source /opt/ros/humble/setup.bash

#Kamera görüntüsünü alıp işlemek (OpenCV ile)
#Bu yapı ile ROS2, Python ve OpenCV entegre bir sistem kurmuş oluyoruz.
...
#MATLAB Simulink – PID Tuning & Real-Time Response
#Sistemlerin blok diyagramla modellenmesi, PID kontrolörlerin grafiksel ayarı (autotune), Gerçek zamanlı step response analizi

###################################################################

#RoboDK / CoppeliaSim (V-REP) – Offline Robot Programlama
#Endüstriyel robotları sanal ortamda simüle etme, Pick & place, kaynak, boyama gibi görevleri test etme, Python üzerinden kontrol komutu gönderme
...
robot.MoveJ(target)  # Joint hareketi
robot.MoveL(target.Pose().TransZ(-200))  # Lineer hareket
...
_, handle = sim.simxGetObjectHandle(clientID, 'UR5_joint1', sim.simx_opmode_blocking)
sim.simxSetJointTargetPosition(clientID, handle, np.pi/2, sim.simx_opmode_oneshot)

###################################################################

#Özet Tablo

#Gazebo -	Fizik bazlı simülasyon - ROS + rclpy
#ROS2	- Gerçek zamanlı mesajlaşma ve kontrol -	rclpy + sensör işleme
#RoboDK	- Endüstriyel robot programlama	- robodk API
#CoppeliaSim	- Robot senaryoları + fizik simülasyon -	sim.py ile bağlantı